...
const ProductArea = () => {
  ...
  const handleActiveTab = (e: any, tab: any) => {
    setActiveTab(tab);
  };
  ...
  if (!isLoading && !isError && products?.data?.length > 0) {
    ...
    if (activeTab === 'All Collection') {
      ...
      product_items = products.data.filter(
        (p: any) => p.category.name === 'Bracelets'
      );
    } else if (activeTab === 'Necklaces') {
      product_items = products.data.filter(
        (p: any) => p.category.name === 'Necklaces'
      );
    } else if (activeTab === 'Earrings') {
      product_items = products.data.filter(
        (p: any) => p.category.name === 'Earrings'
      );
    } else {
      product_items = products.data;
    }
